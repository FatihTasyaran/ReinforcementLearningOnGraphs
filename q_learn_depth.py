...
def q_learn_depth(rewards, framework_graph, graph, q_table, start):
    #print('decay rate:', decay_rate)
    #print('epsilon goes to 0 in:', 1/decay_rate, ' episodes')
    ...
    for episode in range(max_episode):
        max_q_depth = int(max_episode/(episode+1))
...
def main():
    ...
    #####ONE EXPERIMENT RUNTIME Q_LEARN WITH DEPTH#####
    q_table = empty_q_table(graph, alphabet_card)
    rewards = get_rewards(framework_graph)
    q_learn_depth(rewards, framework_graph, graph, q_table, 0)
    #####ONE EXPERIMENT RUNTIME Q_LEARN WITH DEPTH#####
